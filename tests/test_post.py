...
def test_get_one_post_not_exist(authorized_client, test_posts):
    res = authorized_client.get(f"/posts/77777")
...
@pytest.mark.parametrize("title, content, published", [
    ...
    ("new title", "i love pepperoni pizza", False),
    ("skyscraper", "josh", True)
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "title a", "content": "content b"})
    created_post = schemas.PostResponse(**res.json())
    assert created_post.published == True

def test_unauthorized_user_get_create_post(client):
    res = client.post("/posts/", json={"title": "title a", "content": "content b"})
    assert res.status_code == 401
...
def test_delete_post_not_exist(authorized_client, test_posts, test_user):
    res = authorized_client.delete("/posts/777777")
    assert res.status_code == 404
...
def test_update_post_not_exist(authorized_client, test_posts, test_user):
    ...
    res = authorized_client.put("/posts/777777", json= data)
    assert res.status_code == 404
